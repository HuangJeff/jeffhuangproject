...
//修改為實際TPRS server位置 
//var tprs_server_url = "http://localhost/trs"; 
...
function TrsApi(cid,acctId,campaignId) {
	 ...
	 * stock: 庫存量
	 * mainCateName : 大類顯示名稱
	 * middleCateName : 中類顯示名稱
	 * endCateName : 小類顯示名稱
	 * --------------------------------------------------
	 */			
	this.product = function(itemId,mainCate,middleCate,endCate,itemName,newItem,outDate,priority,attributes,listPrice,specialPrice,itemUrl,imgUrl,stock,mainCateName,middleCateName,endCateName) {
		
		//[V2.2新增]		
		mainCateName = checkParameter(mainCateName,"");
		//[V2.2新增]
		middleCateName = checkParameter(middleCateName,"");
		//[V2.2新增]
		endCateName = checkParameter(endCateName,"");
		...
		mainCateName = encodeURI(mainCateName);
		middleCateName = encodeURI(middleCateName);
		endCateName = encodeURI(endCateName);
	 ...
	 * newItem : [V2.2新增]新品取代 : ex: 2 表示前兩個用新品取代; -2表示後兩個用新品取代
	 * -------------------
	 */	
	this.allrcmd = function(mainCate, middleCate, endCate, howMany,titles,targetDivId,newItem) {	
		
		//[V2.2新增]
		newItem = checkParameter(newItem,"0");
	 ...
	 * -------------------
	 */	
	this.catercmd = function(mainCate, middleCate, endCate,userId, hotSale,howMany,titles,targetDivId,newItem) {
	 ...
	 * -------------------
	 */	 	
	this.itemcatercmd = function(mainCate,middleCate, endCate,itemId,hotSale,bonly,howMany,titles,targetDivId,newItem){		
...
function checkParameter(param,defaultValue){	
	if(typeof param == undefined || param == undefined || param == 'undefined' || param == 'null'){
		return defaultValue;
	}
	return param;
}
